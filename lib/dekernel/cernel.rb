class Cernel
  ...
  def ask_which_to_remove
    ...
    $stdout.puts Message.new.installed_kernels(installed_kernels)
  ...
  def purge_packages_from_a_list_of_kernels(kernels_to_remove)
    ...
    if $?.exitstatus != 0
      $stdout.puts Message.new.purge_packages_failure($?)
    else
      $stdout.puts Message.new.purge_packages_success(kernels_to_remove)
  ...
  def confirm_removals(kernels_to_remove)
    ...
    Kernel.system "clear"
    Signal.trap("SIGINT") { $stdout.puts "\nCaught exit signal, exiting!" ; Kernel.exit }
    $stdout.puts Message.new.ask_to_confirm_kernels_to_remove(kernels_to_remove, installed_kernels)
